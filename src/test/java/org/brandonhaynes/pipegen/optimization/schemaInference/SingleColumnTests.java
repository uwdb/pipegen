...
public class SingleColumnTests {
    ...
    public void singleIntegerColumn() throws Exception {
        List<Type> types = ColumnUtilities.inferSchema(new AugmentedString(999, '\n'));
        assert (types.size() == 1);
    ...
    public void singleLongColumn() throws Exception {
        List<Type> types = ColumnUtilities.inferSchema(new AugmentedString(999999999999999L, '\n'));
        assert (types.size() == 1);
        assert (types.get(0) == Long.class);
    }
    ...
    public void singleFloatColumn() throws Exception {
        List<Type> types = ColumnUtilities.inferSchema(new AugmentedString(999.5f, '\n'));
        assert (types.size() == 1);
    ...
    public void singleDoubleColumn() throws Exception {
        List<Type> types = ColumnUtilities.inferSchema(new AugmentedString(999999999999999.5d, '\n'));
        assert (types.size() == 1);
        assert (types.get(0) == Double.class);
    }
    ...
    public void singleStringColumn() throws Exception {
        List<Type> types = ColumnUtilities.inferSchema(new AugmentedString("foo", '\n'));
        assert (types.size() == 1);
